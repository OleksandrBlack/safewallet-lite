...
{
    headers << tr("Avatar")<< tr("Label") << tr("Address") << tr("SafecoinChatAddress") << tr("CID");
    this->parent = parent;
    loadData();
}
    ...
    QObject::connect(ab.addNew, &QPushButton::clicked, [&] () {
        ...
        if (!Settings::isValidAddress(addr)) 
        {
            QMessageBox::critical(
                ...
                QObject::tr("%1 doesn't seem to be a valid safecoin address.").arg(addr), 
                QMessageBox::Ok
            );
            return;
        } 
...
void AddressBook::readFromStorage() 
{
    ...
    if (file.exists()) 
    {
      ...
      //  qDebug() << "Read " << version << " Safecoin contacts from disk...";
        file1.close();
    ...
    {
        qDebug() << "No Safecoin contacts found on disk!";
    }
