...
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-binaries-safewalletlite-v$version.tar.gz
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-deb-safewalletlite-v$version.deb
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-binaries-safewalletlite-v$version.zip
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-installer-safewalletlite-v$version.msi
Remove-Item -Force -ErrorAction Ignore ./artifacts/macOS-safewalletlite-v$version.dmg
...
scp -r src/ res/ ./safewalletlite.pro ./application.qrc ./LICENSE ./README.md ${server}:/tmp/zqwbuild/ | Out-Null
...
if (! (Test-Path ./artifacts/linux-binaries-safewalletlite-v$version.tar.gz) -or
    ! (Test-Path ./artifacts/linux-deb-safewalletlite-v$version.deb) -or
    ! (Test-Path ./artifacts/Windows-binaries-safewalletlite-v$version.zip) -or
    ! (Test-Path ./artifacts/Windows-installer-safewalletlite-v$version.msi) ) {
        Write-Host "[Error]"
        exit 1;
    }
Write-Host "[OK]"
