...
sed -i "s/${PREV_VERSION}/${APP_VERSION}/g" safecoinwallet-lite.pro > /dev/null
...
$QT_STATIC/bin/qmake safecoinwallet-lite.pro -spec linux-clang CONFIG+=release > /dev/null
echo "[OK]"
...
#cat safecoinwallet-lite.pro | sed "s/precompile_header/release/g" | sed "s/PRECOMPILED_HEADER.*//g" > SafecoinWalletLite-mingw.pro
echo "[OK]"
...
cp src/precompiled.h release/
x86_64-w64-mingw32.static-qmake-qt5 safecoinwallet-lite.pro CONFIG+=release > /dev/null
cp src/precompiled.h release/
make -j32 > /dev/null
echo "[OK]"
