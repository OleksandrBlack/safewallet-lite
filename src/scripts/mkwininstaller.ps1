...
Get-Content src/scripts/safewalletlite.wxs | ForEach-Object { $_ -replace "RELEASE_VERSION", "$version" } | Out-File -Encoding utf8 release/wininstaller/safewalletlite.wxs

candle.exe release/wininstaller/safewalletlite.wxs -o release/wininstaller/safewalletlite.wixobj 
if (!$?) {
    exit 1;
}
